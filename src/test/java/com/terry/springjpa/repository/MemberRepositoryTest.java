...
public class MemberRepositoryTest extends AbstractTransactionalTestNGSpringContextTests {
	...
	public void initTest(){
		String sql = "insert into member(idx, loginid, password, name, insertdt, updatedt) "
					+ "values (member_sequence.nextval, ?, ?, ?, current_timestamp(), null)";
		for(int i=0; i < 12; i++){
			jdbcTemplate.update(sql,"로그인 id" + i, "로그인 password" + i, "이름" + i);
		}
	}
	...
	public void 회원등록() throws Exception {
		// Given
		Member member = new Member("로그인 id 등록", "로그인 password 등록", "로그인 이름 등록");
	...
	public void 회원수정() throws Exception {
		...
		// Given
		Member member = new Member("로그인 id5 수정", "로그인 password5 수정", "이름 5 수정");
		member.setIdx(6L);
	...
	public void 회원목록조회_idx정순() throws Exception {
		...
		for(int i=10; i < 12; i++){
			Member member = new Member("로그인 id"+i, "로그인 password"+i, "이름"+i);
	...
	public void 회원목록조회_idx역순() throws Exception {
		...
		List<Member> list = new ArrayList<Member>();
		for(int i=2; i > 0; i--){
			Member member = new Member("로그인 id"+(i-1), "로그인 password"+(i-1), "이름"+(i-1));
	...
	public void 회원로그인아이디Like목록조회_idx정순() throws Exception {
		...
		// Given
		List<Member> list = new ArrayList<Member>();
		Member member1 = new Member("로그인 id1", "로그인 password1", "이름1");
		member1.setIdx(2L);
		Member member2 = new Member("로그인 id10", "로그인 password10", "이름10");
		member2.setIdx(11L);
		Member member3 = new Member("로그인 id11", "로그인 password11", "이름11");
		member3.setIdx(12L);
		...
		// When
		// 1페이지 조회
		Pageable pageable = new PageRequest(0, 10, new Sort(Direction.ASC, "idx"));
